...
Curve* GraphWidget::curve(int iCurve) const
{
...
bool GraphWidget::saveModelScript(const char* szFileName) const
{
	...
	int iModelCurveCount = m_pcrvvCurves.size() - NUM_CAM_CURVES;

	ofsFile << m_fEndTime << std::endl;
	ofsFile << iModelCurveCount << std::endl;
...
bool GraphWidget::saveCameraScript(const char* szFileName) const
{
	std::ofstream ofsFile;
	...
	for (int i = 0; i < NUM_CAM_CURVES; i++, iIndexCameraCurve++) {
		ofsFile << m_ivCurveTypes[iIndexCameraCurve] << std::endl;
		m_pcrvvCurves[iIndexCameraCurve]->toStream(ofsFile);
	}
