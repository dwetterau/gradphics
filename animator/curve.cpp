...
void Curve::removeControlPointsInRange(const float fMinX, const float fMaxX)
{
	for (int i = m_ptvCtrlPts.size()-1; i >= 0  &&  m_ptvCtrlPts.size() > 2; --i) { 
		if( fMinX < m_ptvCtrlPts[i].x  &&  m_ptvCtrlPts[i].x < fMaxX )
		m_ptvCtrlPts.erase(m_ptvCtrlPts.begin() + i);
		m_bDirty = true;
	}
}
