...
void BezierdC::evaluateCurve(const std::vector<Point>& ptvCtrlPts, 
  ...
  for (int i = 0; i < ptvCtrlPts.size() - 3; i += (3 - c)) {
    ...
    k = i + 3;
    helper(p0, p1, p2, p3, &ptvEvaluatedCurvePts);
  }
...
void BezierdC::helper(Point p0, Point p1, Point p2, Point p3,
  ...
  helper(p0, p01, p012, f, ptvEvaluatedCurvePts);
  helper(f, p123, p23, p3, ptvEvaluatedCurvePts);
}
